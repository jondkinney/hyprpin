...
'end',
'',
'-- Forget a tracked window. The one place bookkeeping is dropped, so a',
'-- reservation can never outlive the window it was made for.',
'local function drop(address)',
'  local sv = S.saved[address]',
'  if sv and sv.edge_monitor then unreserve(sv.edge_monitor) end',
'  S.saved[address] = nil',
'end',
'',
...
'end',
'',
'-- `forced` overrides the rule\'s placement: the edge pop hands over here',
'-- with the fallback corner when its display is already docked.',
'local function pop_float(window, rule, forced)',
...
'',
'  local selector = "address:" .. window.address',
'  local placement = forced or rule.placement',
'  local target = nil',
'',
...
'  elseif sv.edge then place_edge(window, sv, rule)',
...
'-- One edge dock per display: a later arrival takes its fallback corner,',
'-- the way a second fill does.',
...
'  end',
'  for other, entry in pairs(S.saved) do',
'    if other ~= window.address and entry.edge_monitor == target.name then',
'      pop_float(window, rule, S.fallback)',
'      return',
'    end',
'  end',
'  local sv = {',
...
'  sv.dock = true',
'  sv.big_prev = nil',
'  -- Tiling into the workspace gives the edge stripe back; undocking or',
'  -- leaving the workspace reserves it again through place().',
'  if sv.edge_monitor then unreserve(sv.edge_monitor) end',
'  hl.dispatch(hl.dsp.window.pin({ window = selector, action = "off" }))',
...
'-- sampling. Corners remember the new size; an edge dock also re-fits its',
'-- stripe around the new thickness (and snaps the other axis back), so the',
'-- tiles follow the window. A window docked into a workspace by SUPER+T is',
'-- sized by the layout, which says nothing about what the user wants, so',
'-- it is skipped. The oneshot chain (instead of a repeat timer) dies with',
'-- its generation, so re-applies never accumulate samplers.',
'local function sample()',
'  if S.gen ~= gen then return end',
'  for address, sv in pairs(S.saved) do',
'    if sv.edge and not sv.big_prev and not sv.tiled then',
...
'        sv.edge_size = (sideways and w.size.x or w.size.y) + between_gap(sv.edge)',
'        if sv.rule_key then',
'          S.sizes[sv.rule_key] = { w = w.size.x, h = w.size.y }',
'          persist_sizes()',
'        end',
'        place_edge(w, sv, eligible(w))',
'      end',
'    elseif not sv.big_prev and not sv.tiled and not sv.special and sv.placement and sv.placement ~= "fill" then',
'      local w = hl.get_window("address:" .. address)',
'      if w and w.pinned and sv.expected_w',
'          and (w.size.x ~= sv.expected_w or w.size.y ~= sv.expected_h) then',
...
'  if S.gen ~= gen then return end',
'  if window and window.address then drop(window.address) end',
'end)',
'',

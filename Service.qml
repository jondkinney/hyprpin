...
Item {
    ...
    function parseSizes(raw) {
        ...
        for (var i = 0; i < parsed.entries.length && out.length < 64; i++) {
            ...
            var scratch = e.scratchpad
            if (scratch && typeof scratch === "object" && !Array.isArray(scratch)
                    && typeof scratch.floating === "boolean"
                    && typeof scratch.x === "number" && isFinite(scratch.x) && scratch.x >= 0 && scratch.x <= 1
                    && typeof scratch.y === "number" && isFinite(scratch.y) && scratch.y >= 0 && scratch.y <= 1
                    && typeof scratch.w === "number" && isFinite(scratch.w) && scratch.w >= 100 && scratch.w <= 16384
                    && typeof scratch.h === "number" && isFinite(scratch.h) && scratch.h >= 60 && scratch.h <= 16384) {
                entry.scratchpad = { floating: scratch.floating, x: scratch.x, y: scratch.y,
                    w: Math.floor(scratch.w), h: Math.floor(scratch.h) }
            }
            out.push(entry)
        }
        return out
    ...
    function sizesLua() {
        ...
        for (var i = 0; i < sizes.length; i++) {
            ...
            var p = s.scratchpad
            var scratchpad = p ? ", scratchpad = { floating = " + (p.floating ? "true" : "false")
                + ", x = " + p.x + ", y = " + p.y + ", w = " + p.w + ", h = " + p.h + " }" : ""
            parts.push("  [" + luaString(s["class"] + "\u001F" + s.title, 520)
                + "] = { w = " + s.w + ", h = " + s.h + floating + scratchpad + " },")
        }
        return parts.join("\n")
    }
...
'if S.special_sub then pcall(function() S.special_sub:remove() end) S.special_sub = nil end',
'S.gen = (S.gen or 0) + 1',
'local gen = S.gen',
...
'      end',
'      local scratchpad = ""',
'      if v.scratchpad then',
'        local p = v.scratchpad',
'        scratchpad = string.format(\', "scratchpad": { "floating": %s, "x": %.6f, "y": %.6f, "w": %d, "h": %d }\',',
'          tostring(p.floating), p.x, p.y, p.w, p.h)',
'      end',
'      local entry = string.format(',
'        \'    { "class": "%s", "title": "%s", "w": %d, "h": %d%s%s }\',',
'        json_escape(class), json_escape(title), v.w, v.h, floating, scratchpad)',
...
'end',
'',
'-- Scratchpad floats remain unpinned and keep geometry independent of pops.',
'local function remember_scratchpad(window, sv)',
'  if not sv.rule_key or not window.monitor or not window.floating',
'      or not window.workspace or window.workspace.name ~= S.special then return end',
'  local w, h = math.floor(window.size.x), math.floor(window.size.y)',
'  if w < 100 or w > 16384 or h < 60 or h > 16384 then return end',
'  local observed, last = observed_float(window), sv.scratchpad_observed',
'  if last and last.x == observed.x and last.y == observed.y and last.w == observed.w',
'      and last.h == observed.h and last.monitor == observed.monitor then return end',
'  sv.scratchpad_observed = observed',
...
'  end',
'  local entry = S.sizes[sv.rule_key] or { w = w, h = h }',
'  local f = { floating = true, w = w, h = h,',
'    x = fraction(window.at.x - ux, uw - w), y = fraction(window.at.y - uy, uh - h) }',
'  local prev = entry.scratchpad',
'  if prev and prev.floating and prev.w == w and prev.h == h and prev.x == f.x and prev.y == f.y then return end',
'  entry.scratchpad = f',
'  S.sizes[sv.rule_key] = entry',
'  persist_sizes()',
'end',
'',
'local function place_scratchpad_float(window, sv)',
'  local m = window.monitor',
'  local uw, uh, ux, uy = usable(m)',
'  local entry = sv.rule_key and S.sizes[sv.rule_key]',
'  local f = entry and entry.scratchpad',
'  -- First float starts near the camera; every later float uses the user\'s rectangle.',
'  local w = math.min(f and f.w or math.floor(uw * 0.5), uw, 16384)',
'  local h = math.min(f and f.h or math.floor(uh * 0.25), uh, 16384)',
'  local x = math.floor(ux + (uw - w) * (f and f.x or 0.5) + 0.5)',
'  local y = math.floor(uy + (uh - h) * (f and f.y or 0.05) + 0.5)',
...
'  sv.scratchpad_observed = nil',
'  remember_scratchpad({ monitor = m, workspace = window.workspace, floating = true,',
'    at = { x = x, y = y }, size = { x = w, y = h } }, sv)',
'end',
'',
'local function remember_window(window, sv)',
'  if sv.special then remember_scratchpad(window, sv) return false end',
...
'-- bookkeeping as a float pop so restore() brings it home by name. Let the',
'-- scratchpad layout size it by default, or restore its remembered float.',
...
'  remember_scratchpad(window, sv)',
'  hl.dispatch(hl.dsp.window.move({ window = selector, workspace = S.special, follow = false }))',
'  local entry = sv.rule_key and S.sizes[sv.rule_key]',
'  if window.workspace and window.workspace.name == S.special then',
'    if entry and entry.scratchpad and entry.scratchpad.floating then',
'      place_scratchpad_float(window, sv)',
'    elseif window.floating then',
'      hl.dispatch(hl.dsp.window.float({ window = selector, action = "off" }))',
'    end',
'  end',
'  -- This flag means docked in an ordinary workspace, not scratchpad tiling.',
'  sv.tiled = false',
'end',
'',
...
'end',
'',
'-- After a compositor reload, a scratchpad window has no runtime bookkeeping.',
'-- Re-adopt only a matching Scratchpad rule, using the visible workspace as home.',
'local function adopt_special(window, rule)',
'  local home = window.monitor and window.monitor.active_workspace',
'  if not rule or rule.placement ~= "special" or not home',
'      or not window.workspace or window.workspace.name ~= S.special then return nil end',
'  local sv = { monitor = window.monitor.name, workspace_id = home.id, workspace_name = home.name,',
'    rule_key = rule_key(rule), rule_placement = rule.placement, rule_monitor = rule.monitor,',
'    placement = "special", placed_at = "special", special = true, floating = window.floating,',
'    fullscreen = window.fullscreen, fullscreen_client = window.fullscreen_client,',
...
'    sv = sv or adopt_special(window, rule)',
'    park_special(window, sv or {})',
...
'  if sv.special or not sv.cycle_slot then remember_window(window, sv) end',
...
'-- stripe. Scratchpad windows toggle tiled/floating there; other pops dock',
'-- into the workspace, then return to their float.',
...
'  if window and not sv then sv = adopt_special(window, eligible(window)) end',
'  if not sv then return false end',
'  sv.cycle_slot, sv.cycle_remaining = nil, nil',
'  local rule = eligible(window)',
'  if sv.special and window.workspace and window.workspace.name == S.special then',
'    sv.dock, sv.tiled, sv.big_prev = false, false, nil',
'    if window.floating then',
'      remember_scratchpad(window, sv)',
'      local entry = sv.rule_key and S.sizes[sv.rule_key]',
'      if entry and entry.scratchpad then entry.scratchpad.floating = false; persist_sizes() end',
'      hl.dispatch(hl.dsp.window.float({ window = "address:" .. address, action = "off" }))',
'    else',
'      place_scratchpad_float(window, sv)',
'    end',
'  elseif sv.edge then',
...
'          end',
'        elseif not sv.special and at_home and not sv.detached and not (rule and rule.stay) then',
'          restore(window, sv)',
'        end',
'      end',
...
'end)',
'',
'-- Capture a last drag when hidden; restore relative geometry when summoned',
'-- on another display. Ordinary scratchpad windows are left to Hyprland.',
'S.special_sub = hl.on("workspace.special_active", function(workspace, monitor)',
'  if S.gen ~= gen or not S.enabled or not monitor then return end',
'  if workspace and workspace.name ~= S.special then return end',
'  for _, window in ipairs(hl.get_windows()) do',
'    if window.monitor and window.monitor.name == monitor.name',
'        and window.workspace and window.workspace.name == S.special then',
'      local sv = S.saved[window.address]',
'      if workspace and not sv then sv = adopt_special(window, eligible(window)) end',
'      if sv and sv.special then',
'        local entry = sv.rule_key and S.sizes[sv.rule_key]',
'        if workspace and entry and entry.scratchpad and entry.scratchpad.floating then',
'          place_scratchpad_float(window, sv)',
'        else',
'          remember_scratchpad(window, sv)',
'        end',
